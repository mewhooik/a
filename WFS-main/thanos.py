...
async def download_video(url, cmd, name):
    ...
    def _build_dl_cmd(base_cmd):
        if is_hls:
            return f'{base_cmd} -R 25 --fragment-retries 25 --concurrent-fragments 16'
        else:
            return f'{base_cmd} -R 25 --fragment-retries 25 --external-downloader "{_aria2c}" --downloader-args "aria2c: -x 16 -j 32"'
