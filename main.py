...
auto_flags = {}
auto_clicked = False
cancel_flags = {}       # chat_id -> True  (set by /cancel)
active_batch_ids = {}  # chat_id -> batch_id (set when a batch starts)
...
def validate_yt_url(url: str):
    """Returns (is_valid, reason). Checks YouTube video ID is exactly 11 chars."""
    m = re.search(r'(?:youtube\.com/watch\?v=|youtu\.be/|youtube\.com/embed/)([A-Za-z0-9_\-]+)', url)
    if m:
        vid_id = m.group(1)
        if len(vid_id) != 11:
            return False, f"Truncated/invalid YouTube video ID `{vid_id}` ({len(vid_id)} chars, need 11). The URL in your file is incomplete."
    return True, ""
...
async def cancel_handler(_, m: Message):
    if m.chat.id not in active_batch_ids:
        await m.reply_text("⚠️ No active batch is running right now.")
        return
    cancel_flags[m.chat.id] = True
    await m.reply_text(
        "⏸️ **Cancel signal sent.**\n"
        "The batch will pause after the current link finishes downloading."
    )

@bot.on_message(filters.command(["freshstart"]))
async def freshstart_handler(_, m: Message):
    _uid = m.from_user.id if m.from_user else m.chat.id
    _pb = batch_resume.get_pending_batch(_uid)
    if _pb:
        batch_resume.mark_cancelled(_pb['id'])
        active_batch_ids.pop(m.chat.id, None)
        await m.reply_text(
            f"🗑️ **Batch cleared:** `{_pb['batch_name']}`\n\n"
            "You can now send a fresh `.txt` file."
        )
    else:
        await m.reply_text("ℹ️ No pending batch to clear.")

@bot.on_message(filters.command(["batchstatus"]))
async def batchstatus_handler(_, m: Message):
    _uid = m.from_user.id if m.from_user else m.chat.id
    _pb = batch_resume.get_pending_batch(_uid)
    if _pb:
        await m.reply_text(
            f"📦 **Batch Status**\n\n"
            f"🏷️ **Name:** `{_pb['batch_name']}`\n"
            f"📍 **Progress:** `{_pb['current_index']}/{_pb['total_links']}` links\n"
            f"🔄 **State:** `{_pb['status']}`\n\n"
            f"Send `/resume` to continue · `/cancel` to pause · `/freshstart` to discard"
        )
    elif m.chat.id in active_batch_ids:
        await m.reply_text("✅ A batch is currently running.")
    else:
        await m.reply_text("ℹ️ No active or pending batch.")

@bot.on_message(filters.command(["resume"]))
async def resume_handler(bot: Client, m: Message):
    """Resume an interrupted batch without re-sending the file."""
    _uid = m.from_user.id if m.from_user else m.chat.id

    if m.chat.id in active_batch_ids:
        await m.reply_text("⚠️ A batch is already running. Send `/cancel` first if you want to switch.")
        return

    _pb = batch_resume.get_pending_batch(_uid)
    if not _pb:
        await m.reply_text("ℹ️ No paused batch found. Nothing to resume.")
        return

    await m.reply_text(
        f"▶️ **Resuming batch…**\n\n"
        f"🏷️ **Name:** `{_pb['batch_name']}`\n"
        f"📍 **From link:** `{_pb['current_index'] + 1}/{_pb['total_links']}`"
    )

    import json as _json
    try:
        links   = _json.loads(_pb['links_json'])
        params  = _json.loads(_pb['params_json'])
    except Exception as _e:
        await m.reply_text(f"❌ Failed to load saved batch data: `{_e}`")
        return

    batch_resume.mark_paused(_pb['id'])   # keep status consistent; loop will set running

    state = {
        'batch_id':        _pb['id'],
        'links':           links,
        'b_name':          _pb['batch_name'],
        'raw_text2':       params.get('raw_text2', '480'),
        'quality':         params.get('quality', '480p'),
        'res':             params.get('res', 'UN'),
        'raw_text3':       params.get('raw_text3', '/d'),
        'PRENAME':         params.get('PRENAME', ''),
        'CR':              params.get('CR', CREDIT),
        'raw_text4':       params.get('raw_text4', '/d'),
        'raw_textx':       params.get('raw_textx', '/d'),
        'watermark':       params.get('watermark', '/d'),
        'raw_text7':       params.get('raw_text7', '/d'),
        'channel_id':      params.get('channel_id', m.chat.id),
        'topic_thread_id': params.get('topic_thread_id'),
        'thumb':           params.get('thumb', '/d'),
        'topic_data':      {},
        'topics_order':    [],
        'start_index':     _pb['current_index'],
        'count_start':     _pb['current_index'] + 1,
        'pdf_count':       0,
        'img_count':       0,
        'v2_count':        0,
        'mpd_count':       0,
    ...
    await _run_batch_loop(bot, m, state)
...
async def status_handler(_, m: Message):
    try:
        ...
        file_count = len([f for f in os.listdir(downloads_dir) if os.path.isfile(os.path.join(downloads_dir, f))]) if os.path.isdir(downloads_dir) else 0
        ...
        await m.reply_text(
            ...
            f"📁 **Files in Downloads:** `{file_count}`\n"
            f"✅ **Bot:** Online"
        )
    except Exception as e:
...
async def start(bot: Client, m: Message):
    ...
    [
        ...
        InlineKeyboardButton("ᴅᴇᴛᴀɪʟꜱ 🦋", callback_data="help")
            ...
            await m.reply_photo(
                photo=photologo,
                caption=f"**Mʏ Nᴀᴍᴇ [DRM Wɪᴢᴀʀᴅ 🦋](https://t.me/mighty_at0m)**\n\n"
                        f"Wᴇʟᴄᴏᴍᴇ, [{m.from_user.first_name}](tg://settings)!\n"
                        f"Hᴇʀᴇ ᴀʀᴇ ᴀʟʟ ᴍʏ ᴄᴏᴍᴍᴀɴᴅꜱ ↓",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("𝕞𝕚𝕘𝕙𝕥𝕪 𝕒𝕥𝕠𝕞", url="https://t.me/mighty_at0m")],
                    [
                        InlineKeyboardButton("ғᴇᴀᴛᴜʀᴇꜱ 🪔", callback_data="help"),
            ...
            commands_text = (
                "**⬇️ Downloads**\n"
                "• /drm — Download DRM-protected course videos\n"
                "• /gdrive — Download from Google Drive\n"
                "• /ytpl — Download a YouTube playlist\n"
                "• /ytm — Download YouTube music\n\n"

                "**📦 Batch Control**\n"
                "• /cancel — Cancel the active batch\n"
                "• /resume — Resume an interrupted batch\n"
                "• /freshstart — Discard saved batch & start fresh\n"
                "• /batchstatus — Check current batch progress\n\n"

                "**⚙️ Configuration**\n"
                "• /setlog — Set the log channel\n"
                "• /getlog — View current log channel\n"
                "• /setgroup — Set the upload group/channel\n"
                "• /getgroup — View current upload group\n"
                "• /removegroup — Remove the upload group\n\n"

                "**🍪 YouTube Cookies**\n"
                "• /cookies — Upload Netscape cookies .txt file\n"
                "• /getcookies — Download the saved cookies file\n"
                "• /mytc — Set YouTube login credentials\n\n"

                "**👤 Account & Info**\n"
                "• /plan — View your subscription details\n"
                "• /ping — Check bot response latency\n"
                "• /status — Show bot & system status\n"
                "• /id — Get current chat or user ID\n\n"

                "**🛠️ Utilities**\n"
                "• /t2t — Convert text to a .txt file\n"
                "• /t2h — Convert text to HTML file\n"
                "• /clean — Clean downloads folder\n"
                "• /stop — Stop the bot\n"
                "• /reset — Reset the bot\n\n"

                "**🎨 Themes**\n"
                "• /modern /neumorphic /brutalist\n"
                "• /glassmorphism /cyberpunk /mellow /yengo\n"
            )

            if is_admin:
                commands_text += (
                    "\n**👑 Admin Only**\n"
                    "• /add — Add a user subscription\n"
                    "• /remove — Remove a user subscription\n"
                    "• /users — List all subscribed users\n"
...
async def _run_batch_loop(bot, m, state: dict):
    """Run the batch download loop. Called by txt_handler and /resume."""
    batch_id     = state['batch_id']
    links        = state['links']
    b_name       = state['b_name']
    raw_text2    = state['raw_text2']
    quality      = state.get('quality', '480p')
    res          = state.get('res', 'UN')
    raw_text3    = state.get('raw_text3', '/d')
    PRENAME      = state.get('PRENAME', '')
    CR           = state.get('CR', CREDIT)
    raw_text4    = state.get('raw_text4', '/d')
    raw_textx    = state.get('raw_textx', '/d')
    watermark    = state.get('watermark', '/d')
    raw_text7    = state.get('raw_text7', '/d')
    channel_id   = state['channel_id']
    topic_thread_id = state.get('topic_thread_id')
    thumb        = state.get('thumb', '/d')
    topic_data   = state.get('topic_data', {})
    topics_order = state.get('topics_order', [])
    start_index  = state.get('start_index', 0)
    count_start  = state.get('count_start', 1)
    pdf_count    = state.get('pdf_count', 0)
    img_count    = state.get('img_count', 0)
    v2_count     = state.get('v2_count', 0)
    mpd_count    = state.get('mpd_count', 0)
    m3u8_count   = state.get('m3u8_count', 0)
    yt_count     = state.get('yt_count', 0)
    drm_count    = state.get('drm_count', 0)
    appx_count   = state.get('appx_count', 0)
    zip_count    = state.get('zip_count', 0)
    ...
    failed_count = 0
    failed_urls  = []
    _batch_cancelled_flag = False
    _bid_loop_i  = start_index

    active_batch_ids[m.chat.id] = batch_id
    ...
    try:
        for i in range(arg-1, len(links)):
            _bid_loop_i = i  # track for crash reporting in except block
            _bid = active_batch_ids.get(m.chat.id)
            # Check if user requested cancel
            if cancel_flags.get(m.chat.id):
                cancel_flags.pop(m.chat.id, None)
                batch_resume.mark_paused(_bid)
                _batch_cancelled_flag = True
                await bot.send_message(m.chat.id,
                    f"⏸️ **Batch paused** at link #{i+1}/{len(links)}.\n"
                    f"Progress saved. Send `/resume` to continue from here."
                )
                break
            # Save progress to DB
            if _bid:
                batch_resume.update_progress(_bid, i)
            # Skip links that have crashed 3+ times
            _link_crashes = batch_resume.get_link_crash_count(_bid, i) if _bid else 0
            if _link_crashes >= 3:
                _skip_name = links[i][0] if links[i] else f'Link #{i+1}'
                await bot.send_message(m.chat.id,
                    f"⏭️ **Skipping link #{i+1}/{len(links)}** — crashed **{_link_crashes}× previously**\n`{_skip_name}`",
                    disable_web_page_preview=True
                )
                failed_urls.append(f"{str(count).zfill(3)}. {_skip_name} [SKIPPED — {_link_crashes} crashes] : https://{links[i][1]}")
                failed_count += 1
                count += 1
                continue
            ...
            _yt_ok, _yt_err = validate_yt_url(url)
            if not _yt_ok:
                await bot.send_message(channel_id, f'⚠️**Invalid YouTube URL**⚠️\n**Name** =>> `{str(count).zfill(3)} {name1}`\n**Url** =>> {link0}\n\n<blockquote><i><b>Failed Reason: {_yt_err}</b></i></blockquote>', disable_web_page_preview=True, message_thread_id=topic_thread_id)
                failed_urls.append(f"{str(count).zfill(3)}. {name1} : {link0}")
                failed_count += 1
                count += 1
                continue
    ...
    f"\n\n<b>🎓  Uᴘʟᴏᴀᴅ Bʏ : {CR}</b>"
        ...
        if _bid:
            try:
                batch_resume.increment_link_error(_bid, _bid_loop_i)
            except Exception:
                pass
            batch_resume.mark_paused(_bid)
        await m.reply_text(
            f"💥 **Batch crashed** on link #{_bid_loop_i+1}.\n"
            f"Progress saved — send `/resume` to continue from here.\n\n"
            f"Error: `{str(e)[:300]}`"
        )
        time.sleep(2)
    else:
        if not _batch_cancelled_flag:
            _bid = active_batch_ids.get(m.chat.id)
            batch_resume.mark_completed(_bid)
            active_batch_ids.pop(m.chat.id, None)
...
async def txt_handler(bot: Client, m: Message):  
    ...
    # Notify user of any interrupted batch
    _bu_id = m.from_user.id if m.from_user else m.chat.id
    _pending_info = batch_resume.get_pending_batch(_bu_id)
    if _pending_info:
        await m.reply_text(
            f"ℹ️ **You have an interrupted batch:**\n"
            f"📦 **{_pending_info['batch_name']}** — "
            f"`{_pending_info['current_index']}/{_pending_info['total_links']}` links done\n\n"
            f"Send `/resume` to continue from link #{_pending_info['current_index']+1} without re-uploading anything.\n"
            f"Or continue sending your file below to start a fresh batch.\n"
            f"Send `/freshstart` to discard the saved batch."
        )
    ...
    f"📌 Send /default → start immediately with all default settings (480p, current chat, no watermark).\n"
    f"Or send start index (1–{len(links)}) to set custom options.**",
)
    ...
    # ── /default shortcut: skip all questions, use sensible defaults ──────────
    if raw_text.strip().lower() == '/default':
        ...
        await editable.edit(
            ...
            f"📦 Batch: `{b_name}`\n"
            f"🎞️ Quality: `480p`  |  📺 Channel: current chat\n\n"
            f"Starting download…"
        )
        await asyncio.sleep(1)
        _bu_id_def = m.from_user.id if m.from_user else m.chat.id
        _def_params = {
            'b_name': b_name, 'raw_text2': raw_text2, 'quality': quality,
            'res': res, 'raw_text3': raw_text3, 'PRENAME': '', 'CR': CR,
            'raw_text4': raw_text4, 'raw_textx': raw_textx, 'watermark': watermark,
            'raw_text7': raw_text7, 'channel_id': channel_id,
            'topic_thread_id': topic_thread_id, 'thumb': thumb,
        }
        _saved_def = batch_resume.get_pending_batch(_bu_id_def)
        if _saved_def:
            batch_resume.mark_cancelled(_saved_def['id'])
        _def_bid = batch_resume.save_batch(_bu_id_def, m.chat.id, b_name, links, _def_params)
        try:
            ...
            pass
        await _run_batch_loop(bot, m, {
            'batch_id': _def_bid, 'links': links, 'b_name': b_name,
            'raw_text2': raw_text2, 'quality': quality, 'res': res,
            'raw_text3': raw_text3, 'PRENAME': '', 'CR': CR,
            'raw_text4': raw_text4, 'raw_textx': raw_textx, 'watermark': watermark,
            'raw_text7': raw_text7, 'channel_id': channel_id,
            'topic_thread_id': topic_thread_id, 'thumb': thumb,
            'topic_data': topic_data, 'topics_order': topics_order,
            'start_index': 0, 'count_start': 1,
            'pdf_count': pdf_count, 'img_count': img_count, 'v2_count': v2_count,
            'mpd_count': mpd_count, 'm3u8_count': m3u8_count, 'yt_count': yt_count,
            'drm_count': drm_count, 'appx_count': appx_count,
            'zip_count': zip_count, 'other_count': other_count,
        })
        return
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # --- Batch Resume: save batch + offer resume if matching interrupted batch ---
    _bu_id = m.from_user.id if m.from_user else m.chat.id
    _batch_params = {
        'b_name': b_name,
        'raw_text2': raw_text2,
        ...
        'res': res,
        'raw_text3': raw_text3,
        'PRENAME': PRENAME if ',' in raw_text3 else '',
        'CR': CR,
        'raw_text4': raw_text4,
        'raw_textx': raw_textx,
        'watermark': watermark,
    ...
    _saved_b = batch_resume.get_pending_batch(_bu_id)
    if _saved_b:
        # Always cancel the old saved batch — user can use /resume instead of re-sending the file
        batch_resume.mark_cancelled(_saved_b['id'])
    _batch_id = batch_resume.save_batch(_bu_id, m.chat.id, b_name, links, _batch_params)
    active_batch_ids[m.chat.id] = _batch_id
    _batch_cancelled_flag = False
    _bid_loop_i = arg - 1  # tracks last attempted index for crash reporting
...
async def text_handler(bot: Client, m: Message):
    ...
    try:
            ...
            _yt_ok, _yt_err = validate_yt_url(url)
            if not _yt_ok:
                await m.reply_text(f'⚠️**Invalid YouTube URL**⚠️\n**Name** =>> `{name1}`\n**Url** =>> {link}\n\n<blockquote><i><b>Failed Reason: {_yt_err}</b></i></blockquote>', disable_web_page_preview=True)
                return
...
async def _startup_notify():
    """Auto-resume all batches that were interrupted by a bot crash/restart."""
    await asyncio.sleep(5)  # let the bot fully connect first
    interrupted = batch_resume.get_all_running_batches()
    for _b in interrupted:
        try:
            import json as _json
            params = _json.loads(_b['params_json'])
            links  = _json.loads(_b['links_json'])

            await bot.send_message(
                _b['chat_id'],
                f"🔄 **Bot restarted — auto-resuming your batch...**\n\n"
                f"📦 **{_b['batch_name']}**\n"
                f"▶️ Continuing from link **#{_b['current_index']+1}/{_b['total_links']}**"
            )

            # Build a lightweight fake Message so _run_batch_loop can send replies
            class _FC:
                def __init__(self, cid): self.id = cid
            class _FM:
                def __init__(self, cid):
                    self.chat      = _FC(cid)
                    self.from_user = _FC(cid)
            ...
            fake_m = _FM(_b['chat_id'])

            state = {
                'batch_id':        _b['id'],
                'links':           links,
                'b_name':          _b['batch_name'],
                'raw_text2':       params.get('raw_text2', '480'),
                'quality':         params.get('quality', '480p'),
                'res':             params.get('res', 'UN'),
                'raw_text3':       params.get('raw_text3', '/d'),
                ...
                'watermark':       params.get('watermark', '/d'),
                'raw_text7':       params.get('raw_text7', '/d'),
                'channel_id':      params.get('channel_id', _b['chat_id']),
                'topic_thread_id': params.get('topic_thread_id'),
                'thumb':           params.get('thumb', '/d'),
                'topic_data':      {},
                'topics_order':    [],
                'start_index':     _b['current_index'],
                'count_start':     _b['current_index'] + 1,
            }
            await _run_batch_loop(bot, fake_m, state)

        except Exception as _ne:
            print(f"[StartupNotify] Auto-resume failed for batch {_b['id']}: {_ne}")
            try:
                batch_resume.mark_paused(_b['id'])
                await bot.send_message(
                    _b['chat_id'],
                    f"⚠️ **Auto-resume failed for** `{_b['batch_name']}`.\n"
                    f"Send `/resume` to try manually."
                )
            except Exception:
                pass
...
async def _bot_main():
    await bot.start()
    asyncio.get_event_loop().create_task(_startup_notify())
    await idle()
    await bot.stop()

while True:
    try:
        asyncio.get_event_loop().run_until_complete(_bot_main())
        break
    except Exception as _e:
        _err = str(_e)
        import re as _re
